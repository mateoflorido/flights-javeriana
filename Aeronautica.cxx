    ...
    if (itRoutes != this->m_Routes.end()) {
        ...
        for (; itAgencies != this->m_Agencies.end(); itAgencies++) {
            if (itAgencies->GetAgencyID() == m_Agency) {
                itAgencies->NewSale(m_Agency, m_ID, m_Flight, m_CustomerID, m_Customer, m_FlightDate, m_BuyDate,
                                    m_BuyHour);
                return;
            }//fi
        }//rof
...
::GraphInitialize() {
    ...
    for (; RIt != this->m_Routes.end(); RIt++) {
        ...
        this->m_GRoutes.AddArc(this->m_GRoutes.GetIndex((*RIt).GetOrigin()),
                               this->m_GRoutes.GetIndex((*RIt).GetDestination()), (*RIt).GetDuration());
    }
}
...
bool
FJA::Aeronautica
::ChangeSale(std::string IDPassenger, std::string IDFlightO, std::string IDFlightN, std::string IDAgency,
             std::string buyHour, std::string buyDate, std::string date, float &newPrice) {
    auto ItAgency = this->m_Agencies.begin();
    for (; ItAgency != this->m_Agencies.end(); ItAgency++) {
